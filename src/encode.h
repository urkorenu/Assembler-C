 ...
 * integer type number the bits we want to add,
 * and integer type nuber the position where we want to add the bits.
 ...
 * @param found_register An integer flag indicating whether a register operand was found in the instruction. 
 * @param source_code A pointer to a struct linked_list 
    representing the source code data associated with the instruction.
 * @param source An integer flag indicating whether the register operand is from the source
    or destination of the instruction.
 */
extern void
 ...
 * @brief Encodes direct addressing operands in assembly instructions
   This function Determines whether the operand is from the source or destination of the instruction.
   Parses and converts the operand value to an integer, shifting it appropriately.
   Inserts the encoded value into the assembler's object list, updating the instruction counter and object list count.
   Handles errors related to invalid values, undefined symbols, or unknown data.
 * @param assembler: Pointer to a struct assembler_data.
 * @param inst: A pointer to a struct line_data.
 * @param source_code A pointer to a struct linked_list
   representing the source code data associated with the instruction
 * @param source An integer flag indicating whether the operand is from the source or destination of the instruction.
 */
extern void
 ...
 * @brief Encodes indexed addressing operands in assembly instructions.
   This function determines whether the operand is from the source or destination of the instruction.
   Encodes the source code data (such as opcode or address) with the index address.
   Inserts a placeholder value into the assembler's object list for later modification during the second phase.
   Parses and converts the index value to an integer, shifting it appropriately.
   Inserts the encoded index value into the assembler's object list, updating the instruction counter and object list count.
   Handles errors related to invalid values, undefined symbols, or unknown indices.
 * @param assembler: Pointer to a struct assembler_data.
 * @param inst: A pointer to a struct line_data.
 * @param source_code A pointer to a struct linked_list
   representing the source code data associated with the instruction
 * @param source An integer flag indicating whether the operand is from the source or destination of the instruction.
 * @param index A pointer to a character array representing the index value extracted from the instruction.
 */
extern void
 ...
 * @brief Encodes null (empty) operands in assembly instructions.
   This function Determines whether the operand is from the source or destination of the instruction.
   Encodes the source code data (such as opcode or address) with a direct address value.
   Sets the encoded value into the source code data.
   Inserts a placeholder value (0) into the assembler's object list for later modification.
   Updates the instruction counter and object list count.
 * @param assembler: Pointer to a struct assembler_data.
 * @param inst: A pointer to a struct line_data.
 * @param source_code A pointer to a struct linked_list
   representing the source code data associated with the instruction
 ...
 * @brief Encodes the operand of an assembly instruction based on its type.
   This function determines whether the operand is a register, immediate value, indexed addressing, or null.
   Calls corresponding encoding functions based on the operand type.
   Manages the state of found registers during encoding.
 * @param assembler: Pointer to a struct assembler_data.
 * @param inst: A pointer to a struct line_data.
 * @param source_code A pointer to a struct linked_list
   representing the source code data associated with the instruction
 * @param operand A character pointer representing the operand of the instruction that needs to be encoded.
 * @param is_source An integer flag indicating whether the operand is from the source or destination of the instruction.
 * @param found_reg A pointer to an integer used to track whether a register operand has been found during encoding. 
 */
extern void
