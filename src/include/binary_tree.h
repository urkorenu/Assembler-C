...
struct binary_tree {
    struct tnode*
      root; /* Holds a pointer to the root node of the binary tree. */
};
...
/**
 * @brief This function retrieves data associated with the given key from a
 * binary tree.
 * @param tree The binary tree containing the data.
 * @param key The key to search for in the binary tree.
 * @return Returns a pointer to the data associated with the given key if found,
...
/**
 * @brief This function prints the contents of a binary tree by its key and
 * associated data.
...
/**
 * @brief This function inserts a node with the given key and data into the
 * binary tree.
 ...
 * @return Returns an integer value indicating the success of the operation
 * (always returns 1).
 */
extern int
 ...
 * @brief This function inserts a node with the given key and data into the
 * binary tree.
 * @param p A pointer to the root node of the binary tree.
 * @param key The key associated with the node being inserted.
 * @param data The data associated with the node being inserted.
 * @return Returns a pointer to the root of the binary tree after inserting the
 * new node.
 */
extern struct tnode*
...
/**
 * @brief This function allocates memory for a new binary tree structure and
 * initializes its root pointer to NULL.
 ...
 * @pararam is_bucket An int that indicates if there is a bucket that needs to get freed.
 */
extern void
btree_free(struct binary_tree* btree, int is_bucket);

/**
 * @brief This function recursively frees the memory allocated for each node of
 the binary tree, including the associated data stored in the buckets.
 * @param p A pointer to the root node of the binary tree.
 * @pararam is_bucket An int that indicates if there is a bucket that needs to get freed.
 */
extern void
free_tree(struct tnode* p, int is_bucket);
