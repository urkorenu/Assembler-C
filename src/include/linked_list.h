...
#define BITSIZE(x)                                                             \
    (sizeof(x) * 8) /* Calculates the size of a variable x in bits. */
#define BITMASK(N)                                                             \
    (1 << (N)) /* Generates a bitmask for a specific bit position N. */
#define GET_BIT(X, N)                                                          \
    (((X) & BITMASK(N)) >>                                                     \
     (N)) /* Retrieves the value of a specific bit N from an integer X. */

/* Represents the state of a node in a linked list. */
enum node_state { DATA_UNSET, DATA_SET };

struct linked_list {
    void* data; /* Stores the data associated with the current node in the
                   linked list. */
    enum node_state
      state; /* The state of the current node in the linked list. */
    struct linked_list*
      next; /* A pointer to the next node in the linked list. */
    struct linked_list*
      pre; /* a pointer to the previous node in the linked list. */
};
...
/**
 * @brief This function sets the data pointer of a linked list node to a
 * specified value and updates its state to indicate that the data has been set.
...
/**
 * @brief This function inserts a new node containing the provided data at the
 * end of the linked list.
 * @param  head A pointer to the head of the linked list.
 * @param  last_node A pointer to the last node of the linked list.
 * @return Returns a pointer to the newly inserted node at the end of the linked
 * list.
 */
extern struct linked_list*
...
/**
* @brief This function traverses the linked list starting from the provided node
p and returns the first node with its data state unset.
* @param p A pointer to the current node from which the traversal begins.
* @param ic A pointer to the instruction counter, which is incremented for each
node visited.
* @return Returns  a pointer to the first node with its data state unset in the
linked list.
*/
extern struct linked_list*
...
/**
 * @brief This function recursively frees the memory allocated for each node in
 * the linked list
 * @param p A pointer to the current node from which the traversal begins.
 * @param free_b Int reperset whether a bucket needs to be freed.
 */
extern void
llfree(struct linked_list* p, int free_b);

/**
 * @brief This function prints the contents of the object list linked list to a
 * file.
 * @param assembler A pointer to the assembler data structure containing
 ...
 * @param file File pointer representing the file where the elements will be
 * printed.
