...
static int is_ended_with_x(char *word, char x) {
  int length = strlen(word);
  if (word[length - 1] == x)
    return 1;
  return 0;
...
static int is_starting_with_x(char *word, char x) {
  if (word[0] == x)
    return 1;
  return 0;
}
...
int line_to_bin_1st(struct assembler_data *assembler, char *line,
  ...
  if ((word = get_word(line, idx_ptr))) {
    if (is_symbol(word)) {
      ...
      word = get_word(line, idx_ptr);
    } else if ((word = get_word(line, idx_ptr)))
...
struct bucket *parse_first_phase(struct assembler_data *assembler,
                                 FILE *source_file) {
  int dc = 100;
  int ic = 100;
  char line[MAXWORD];
